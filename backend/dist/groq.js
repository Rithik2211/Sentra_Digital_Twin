...
exports.generateSupervisorBriefing = generateSupervisorBriefing;
exports.getMockBriefing = getMockBriefing;
...
async function generateSupervisorBriefing(runId, scenario, activeAlarms) {
    // Construct a deterministic prompt string
    const promptInput = `Scenario: ${scenario}. Active Alarms: ${activeAlarms.sort().join(', ') || 'None'}.`;
    // Calculate MD5 hash
    const hash = crypto_1.default.createHash('md5').update(promptInput).digest('hex');
    const cacheKey = `briefing:${hash}`;
    // Check Redis cache
    const cachedBriefing = await (0, redis_1.cacheGet)(cacheKey);
    if (cachedBriefing) {
    ...
    await (0, redis_1.cacheSet)(cacheKey, briefing, 900);
    return { briefing, cached: false };
}
function getMockBriefing(scenario, activeAlarms) {
