...
export class SimulationEngine {
  private static activeRuns = new Map<string, {
  ...
  static getActiveRun(runId: string) {
    return this.activeRuns.get(runId);
  }

  static async start(tenantId: string, scenario: string, speed: number): Promise<any> {
    ...
    if (isDbConnected) {
      ...
      memoryStore.simulationRuns.push({
        ...runData,
        endedAt: null
      });
    }
    ...
    const interval = setInterval(async () => {
      ...
      if (isDbConnected) {
        try {
          ...
          for (const w of state.workers) {
            await prisma.workerAgent.upsert({
              where: { id: w.id },
          ...
          for (const ev of state.riskEvents) {
            const exists = await prisma.riskEvent.findFirst({ where: { id: ev.id } });
            if (!exists) {
        ...
        // Sync memory store
        for (const w of state.workers) {
          const idx = memoryStore.workerAgents.findIndex(mw => mw.id === w.id);
          if (idx >= 0) {
            memoryStore.workerAgents[idx] = { ...w, simulationRunId: runId };
          } else {
            memoryStore.workerAgents.push({ ...w, simulationRunId: runId });
          }
        }
        for (const s of state.sensors) {
          const idx = memoryStore.sensors.findIndex(ms => ms.id === s.id);
          if (idx >= 0) {
            memoryStore.sensors[idx].lastReading = s.lastReading;
            memoryStore.sensors[idx].status = s.status;
            memoryStore.sensors[idx].updatedAt = new Date();
          }
        }
        for (const z of state.zones) {
          const idx = memoryStore.zones.findIndex(mz => mz.id === z.id);
          if (idx >= 0) {
            memoryStore.zones[idx].riskLevel = z.riskLevel;
          }
        }
        for (const ev of state.riskEvents) {
          const exists = memoryStore.riskEvents.some(mre => mre.id === ev.id);
          if (!exists) {
            memoryStore.riskEvents.push({
              id: ev.id,
              simulationRunId: runId,
              zoneId: ev.zoneId,
              severity: ev.severity,
      ...
      await publishMessage(`simulation:${runId}`, JSON.stringify(tick));
  ...
  static async stop(runId: string): Promise<any> {
    ...
    if (isDbConnected) {
      ...
      const idx = memoryStore.simulationRuns.findIndex(sr => sr.id === runId);
      if (idx >= 0) {
        memoryStore.simulationRuns[idx].status = 'COMPLETED';
        memoryStore.simulationRuns[idx].endedAt = endedAt;
        return memoryStore.simulationRuns[idx];
      }
    }
    return null;
  }

  static getRunHistory(runId: string): TickData[] {
    const run = this.activeRuns.get(runId);
  ...
  private static getZoneDefaultCoords(zoneId: string) {
    switch (zoneId) {
  ...
  private static computeNextState(
    ...
    const moveWorker = (w: any, targetZoneId: string, speedFactor = 0.4) => {
